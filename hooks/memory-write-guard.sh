...
deny() {
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":%s}}\n' \
        "$(printf '%s Fix it and write again. The conventions and a template are at %s' "$1" "$GUIDE" | jq -Rs .)"
    exit 0
}
...
case "$slug" in
    ...
    *) deny "A memory filename must start with user_, feedback_ or project_, and '$base' does not. Global preferences are feedback_, facts about the user are user_, and project-specific notes are project_ and belong in memory-mounts." ;;
esac
...
if has '^name:'; then
    ...
    [ "$name" = "$slug" ] || deny "The name field must equal the filename slug: '$slug', not '$name'. The generated index and every [[wikilink]] resolve through it, and the harness suggests a kebab-case name that breaks both."
fi

# 3, 4, 5. whole-file rules: only a Write shows enough to judge absence
if [ -n "$whole" ]; then
    has '^description:' || deny "A memory file needs a description line in its frontmatter. It is the index entry and what recall reads to judge whether the file is relevant, so a file without one is close to invisible."
    has '^[[:space:]]*type:' || deny "A memory file needs a type in its metadata: user, feedback, project or reference."
    case "$slug" in
        feedback_*)
            has '^\*\*Why:\*\*' || deny "A feedback rule needs a **Why:** line giving the generalized reason it matters. Without it the rule reads as an arbitrary instruction and gets ignored or misapplied later." ;;
    esac
fi
...
case "$fp" in
    "$(mk_memory_dir)"/*)
        if has '^\*\*Evidence' || has '^#\{1,6\}[[:space:]]*Evidence'; then
            deny "Global memory files carry no Evidence section. Quotes, repo names and paths stay in the miner tracker and git history; distil the lesson into Why and How instead. This file syncs to a personal GitHub repo, which is why the leak surface was removed rather than policed."
        fi ;;
esac
